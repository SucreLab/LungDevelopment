...
N_WORKERS <- 3
options(future.globals.maxSize=11*1024*1024^2) # First number is GB of memory allowed
...
pdf("./figures/meso/supp_umap.pdf", width = 12, height = 4.5)
p_cluster + p_time + plot_annotation("Mesenchyme")
dev.off()
```
...
pdf("./figures/meso/supp_cluster_markers.pdf", height = 5.5, width = 12)
meso_heatmap
dev.off()
```
...
text_vec <- list(c("Wnt2+", 2),
                 ...
                 c("Smooth\nmuscle", 2),
                 c("Neuron", 2))
levels(Idents(meso_relabel)) <- c("Wnt2+", #0
                                  "Myofibroblast", #1
                                  "Adventitial fibroblast", #2
                                  "Prenatal Wnt2+", #3
                                  "Pericyte", #4
                                  "Prenatal Myofibroblast", #5
                                  "Mesothelium", #6
                                  "Smooth muscle", #7
                                  "Neuron" #8

)
Idents(meso_relabel) <- ordered(Idents(meso_relabel), c("Prenatal Wnt2+", "Wnt2+", "Prenatal Myofibroblast", "Myofibroblast", "Adventitial fibroblast", "Pericyte", "Mesothelium", "Smooth muscle", "Neuron"))
...
pdf("./figures/meso/umap.pdf", width = 6, height = 4.5)
p_rename_cluster + plot_annotation("Mesenchyme")
dev.off()
```
...
### Prenatal Wnt2+
```{r}
meso_relabeled_markers[[0 + 1]]
```

### Wnt2+
```{r}
meso_relabeled_markers[[1 + 1]]
```
### Prenatal Myofibroblast
```{r}
meso_relabeled_markers[[2 + 1]]
```

### Myofibroblast
```{r}
meso_relabeled_markers[[3 + 1]]
```

### Adventitial fibroblast
```{r}
meso_relabeled_markers[[4 + 1]]
```

### Pericyte
```{r}
meso_relabeled_markers[[5 + 1]]
```

### Mesothelium
```{r}
meso_relabeled_markers[[6 + 1]]
```

### Neuron
```{r}
meso_relabeled_markers[[7 + 1]]
```
...
pdf("./figures/meso/labeled_mountain.pdf", width = 9, height = 4)
meso_mountain
dev.off()
...
pdf("./figures/meso/meso_wnt_plot.pdf", width = 12, height = 4.5)
wnt2_meso_plot + wnt5a_meso_plot + plot_layout(ncol = 2)
dev.off()
```
...
myo_highlight <- ggplot() +
        geom_point(data = ggplot_data[!(ggplot_data$named_clusters %in% c("Prenatal Myofibroblast", "Myofibroblast")),],
...
pdf("./figures/meso/umap_myo_timepoint.pdf", width = 5, height = 4.5)
myo_highlight
dev.off()

pdf("./figures/meso/umap_myo_supp_cluster.pdf", width = 6, height = 4.5)
myo_supp_highlight
dev.off()
...
### Cluster 0
```{r}
meso_markers_highres[[0 + 1]]
```

### Cluster 1
```{r}
meso_markers_highres[[1 + 1]]
```

### Cluster 2
```{r}
meso_markers_highres[[2 + 1]]
```

### Cluster 3
```{r}
meso_markers_highres[[3 + 1]]
```

### Cluster 4
```{r}
meso_markers_highres[[4 + 1]]
```

### Cluster 5
```{r}
meso_markers_highres[[5 + 1]]
```

### Cluster 6
```{r}
meso_markers_highres[[6 + 1]]
```

### Cluster 7
```{r}
meso_markers_highres[[7 + 1]]
```

### Cluster 8
```{r}
meso_markers_highres[[8 + 1]]
```

### Cluster 9
```{r}
meso_markers_highres[[9 + 1]]
```

### Cluster 10
```{r}
meso_markers_highres[[10 + 1]]
```

### Cluster 11
```{r}
meso_markers_highres[[11 + 1]]
```

### Cluster 12
```{r}
meso_markers_highres[[12 + 1]]
```
