...
```{r}
car4_plot <- FeaturePlot(endo, c("Car4")) +
  featureplot_theme() +
  theme(aspect.ratio=1,
...
apln_plot <- FeaturePlot(endo, c("Apln")) +
  featureplot_theme() +
  theme(aspect.ratio=1,
        plot.title = element_text(face = "italic", size = 14),
        legend.title = element_text(size = 14),
...
car4_plot + apln_plot + plot_layout(ncol = 2)
```

```{r}
FeaturePlot(endo, c("Ackr3")) +
  featureplot_theme() +
  theme(aspect.ratio=1,
        plot.title = element_text(face = "italic", size = 14),
        legend.title = element_text(size = 14),
...
endo_dfrank <- endo
# Make the tiempoints an ordered factor, so they are displayed consistantly
endo_dfrank$timepoint <- ordered(as.factor(endo_dfrank$timepoint), unique(endo_dfrank$timepoint))

endo_dfrank <- cluster_pca_umap(endo_dfrank, k_param = 20, dims_umap = 1:20, dims_neighbors = 1:20, cluster_res = 1) # Note, this breaks if future is set to plan: multiprocess


p_df_cluster <- DimPlot(endo_dfrank) + umap_theme() +
  scale_colour_manual(name = "Cluster", values = color_category_20) +
  theme(aspect.ratio=1)
p_df_time <- DimPlot(endo_dfrank, group.by = "timepoint") + umap_theme() +
  scale_colour_manual(name = "Timepoint", values = color_scanpy_default)+
  theme(aspect.ratio=1)

p_df_cluster + p_df_time + plot_annotation("Endothelium")

```


```{r}
DimPlot(subset(endo_dfrank, timepoint == "E12")) + umap_theme() +
  scale_colour_manual(name = "Timepoint", values = color_category_20)+
  theme(aspect.ratio=1)
```

```{r}
intermediate_cell_markers <- FindMarkers(endo_dfrank, 16, NULL, test.use = "MAST")
#intermediate_cell_markers$gene <- rownames(intermediate_cell_markers)
intermediate_cell_markers
write.xlsx(intermediate_cell_markers, file = "/Users/negretn/postdoc/markers_art_vein_intermediate.xlsx", rowNames = TRUE)
```
...
```{r}
ggsave("./for_alice_apln.tiff", p_rename_cluster + endo_mountain + car4_plot + apln_plot +  plot_layout(ncol = 2), scale = 1.25)
```
