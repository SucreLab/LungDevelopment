...
all_cells$cell_subtype[all_cells$cell_subtype == "Prenatal Myofibroblast"] <- "Proliferating Myofibroblast"
...
pdf("./figures/all/all_umap_nolines.pdf", width = 11, height = 7)
all_umap
dev.off()
...
tiff(paste0("./figures/all/time_umap/",timepoint_str,"_umap.tiff"), width = 12, height = 12, unit = "cm", res = 600, compression = "lzw")
print(all_umap)
dev.off()
}
...
```{r}
primordial_markers <- FindMarkers(all_cells, "Primordial", NULL, group.by = "cell_subtype", test.use = "MAST")
primordial_markers[n_print,]
```
...
# Note - the next few lines combines the prenatal Wnt2+ and the Prenatal Myofibroblasts
...
pdf("./figures/all/e15_highlight_UMAP_revised.pdf", width = 9, height = 7)
e15_highlight
dev.off()
...
pdf("./figures/all/p3_highlight_UMAP_revised.pdf", width = 9, height = 7)
p3_highlight
dev.off()
```
...
pdf("./figures/all/UMAP_time.pdf", width = 9, height = 7)
all_umap_bytime
dev.off()
```
...
Idents(all_cells) <- renameFactorIdent(Idents(all_cells), "Proliferating Myofibroblast", "Proliferating myofibroblast")
...
pdf("./figures/all/heatmap_allgenes.pdf", width = 9, height = 7)
all_heatmap
dev.off()
...
pdf("./figures/meso/myo_matrix.pdf", width = 12, height = 4)
myo_matrix_plot
dev.off()
```
...
pdf("./figures/all/dotmap_matrix_all.pdf", width = 9, height = 7)
all_mat_dot_heatmap
dev.off()
...
pdf("./figures/all/meso_mountain.pdf", width = 9, height = 4)
meso_mountain
dev.off()
```
```{r}
...
pdf("./figures/all/epi_mountain.pdf", width = 9, height = 4)
epi_mountain
dev.off()
```
```{r}
...
pdf("./figures/all/endo_mountain.pdf", width = 9, height = 4)
endo_mountain
dev.off()
```

```{r}
pdf("./figures/all/all_mountains.pdf", width = 8, height = 12)
